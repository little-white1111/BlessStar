...
| **XX-CONC-6** | 第20天范围 | ✅ | 保证 **并发读配置 + 单写者 reload**；**不**承诺多线程并行 **`bs_kernel_execute`**（execute 须写锁或单线程 executor）。 |
...
| T20.6 | **IMPL-20-CONC-06** | **ordered notify worker** + `flush` on destroy | 🟡 | MVP：`emit_transition` 二阶段；**二期** `attach_notify_queue.cpp` |
...
| 落地项 | （待子任务 B 填写） |
| 实现文件 | （待） |
| 测试 | `ctest -L day20` **1/1**（Windows Release）；Linux **tsan** job 待 Actions |
| 验证结果 | （待）TSan / 并发测例 |
| 台账 | （待）`项目修改记录.md` 变更 **20.x** |
| 遗留 | 见 **刻意未定义项** |
...
| 未定义项 | 说明 |
|----------|------|
| 多线程并行 `bs_kernel_execute` | **XX-CONC-6**；二期 Kernel 内锁或单线程 executor |
...
| 下一步 | 主任务下发子任务 B：**T20-0→T20-1→T20-2→T20-3**；完工后更新 § 方案状态 |
