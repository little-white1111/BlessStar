...
| T-POOL.18 | **IMPL-21-CI-01** | Linux **tsan** 增 `kernel_pool` 标签 | ✅ | `.github/workflows/ci.yml` |
...
- **子任务 B**：✅ **T-POOL-0～20 已落地**（Windows `ctest -L regression` **82/82**；Linux TSan 待 CI 取证）  
- **状态**：✅ 已完全落地（MVP · A′′′）；**Linux TSan 取证**见 **用户补充裁定 · CI 流程**  
...
| **Sanitizer 真相源** | **Linux GitHub Actions** `ci.yml` → job **`tsan (Linux Clang day20)`**；**IMPL-21-CI-01** 闭合范围 **`ctest -L kernel_pool`** |

#### 子任务B 落地状态总结（第21天）

| 类别 | 内容 |
|------|------|
| 落地项 | **T-POOL-0～20** 全闭合：契约、A′′ executor、KernelPool、IrSnapshot、reload Phase A/B/C、`attach_exec` 委托 |
| 实现文件 | `kernel_pool.c` / `kernel.c` / `attach_ir_snapshot.cpp` / `reload_batch_controller.cpp` / `attach_execute.cpp` / `registry_bootstrap.cpp` |
| 测试 | `ctest -L kernel_pool` **3/3**；`KernelExecutorTest`；`ReloadPerBatchParallelExecTest`；`ReloadIrExecuteIntegrationTest` |
| 验证结果 | Windows `ctest -L regression` **82/82**；`day20` **2/2**；`kernel_pool` **3/3**；契约 validate+compile ✅；**Linux TSan**：待 Actions `tsan` job 绿跑后填 run URL |
| 台账 | `项目修改记录.md` 变更 **21.1～21.7** |
...
| IMPL-21-CI-01 | **T-POOL.18** · Linux tsan kernel_pool | 第21天 | ci | 🟡 | `ci.yml` tsan · `-L kernel_pool` | PR Actions run URL | 第21天 § **T-POOL-4** |
