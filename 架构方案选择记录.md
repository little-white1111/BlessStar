...
| RES-IX-17 | store 长驻 + controller session reset | ✅ | **第22天 RS′**（2026-06-04 用户确认）：**`BsAttachStore` 挂 `AttachContext`**（L1）；**`reload_batch_reset`** 清空 L2 暂存；**`destroy(ctrl)` 不 close store**；禁止 controller 对象池；**iter = run 次数**（**XIX-MEM**）；详见 § 第22天 **RS′** |
...
| RES-G-04 | 读者未绑持久层 | 🟡 | commit 前不可见；manifest revision 绑读者待 **T-REC-G03-4** | § 第22天 **REC-G-03 闭合** |
...
| RES-D-05 | 持久文件与运行时 IR 双真相 | 🟡 | Step-② 已部分缓解；完整闭合见 **§ REC-G-03 闭合** · **T-REC-G03** | **WIRE** + state 同步 + **T-REC-G03** |
...
| T19.12 | — | 契约 | ✅ | **用户裁定（2026-06-04·T19.12-C）**：**`C-TST-MEM-1` rule-only**；**不**建 blocking **`GATE-TEST-DAY19-SMOKE`**；执行仍以 CMake 标签 + GHA 为准；见 § 第22天 |
...
| **XX-CONC-5** | 销毁序 | ✅ | `closing` → 停新 guard → `flush_notify_queue` → 等 inflight 写 → 释放 arc/锁。 |
...
- **状态**：✅ **已完全落地**（MVP · B″）；Linux **tsan** 已由第21天 **`full test`** / **`day21`** Actions 取证 ✅  
...
| 验证结果 | ✅ `check_attach_lock_order.py`；✅ `GATE-ARCH-INCLUDE-BOUNDARY`；✅ Linux **`full test` tsan**（第21天 CI 链） |
| 台账 | `项目修改记录.md` 变更 **20.1**、**20.2** |
| 遗留 | （无 · **XX-CONC-6 execute 并行** 已由 **第21天 A′′′** 闭合） |
...
> **状态**：⭐ **方案 A′′′（A′′ + KernelPool）** 及 **KERNEL-CONC-1～7 / KERNEL-POOL-1～10 / IR-SNAPSHOT-1～3 / ORCH-POOL-1～3** 已用户确认（2026-06-04）；子任务 B **T-POOL-0～21 已落地**；**第21天已验收**（2026-06-04）；**full test** ✅。  
...
- **子任务 B**：✅ **T-POOL-0～21 已落地**；**`full test` + `day21`** CI 全绿；PR #1 已 squash merge  
- **状态**：✅ **已完全落地**（MVP · A′′′）；**Linux TSan** + **全仓 regression** + **day19 ci stress** 均已 Actions 取证  
...
| 类别 | 内容 |
|------|------|
| 落地项 | **T-POOL-0～21** 全闭合：契约、A′′ executor、KernelPool、IrSnapshot、reload Phase A/B/C、`attach_exec` 委托；CI 分层；attach_watch 锁外回调；Debug reentrant；**P2 attach 实例化**；**arch_gap** 测例；**day19 ci 预算** |
| 实现文件 | 上表 + `attach_context.cpp`（TLS ctx stack）· `attach_watch.c` · `attach_notify_queue.cpp` · `.github/workflows/day21.yml` |
| 测试 | `ctest -L kernel_pool` **3/3**；`AttachConcurrencyTest`；`AttachP2ShortcomingRegressionTest`（`-L arch_gap`）；`day19` ci stress ~26s |
| 验证结果 | **`full test`** **6/6 jobs** ✅ [#27070111361](https://github.com/little-white1111/BlessStar/actions/runs/27070111361)（最终合并验收）；**`day21`** ✅ [#27058137337](https://github.com/little-white1111/BlessStar/actions/runs/27058137337) |
| 台账 | `项目修改记录.md` 变更 **21.1～21.16** |
| 遗留 | **无工程缺口**；execute 超时/协作式 cancel 为 **刻意未定义**；Windows TSan 非门禁；**T19.12** → **第22天** |
...
| **R21-05** | P2 attach TLS / 写窗口与 pool 正交 | 并行 log/reload 错 ctx | **21.12～21.14** | ✅ | **arch_gap** 测例 |
...
| 未决（工程） | （无 · **R21-01～05** 已闭合） |
| 下一步 | 进入 **第22天** · 全量回归自动化 + **T19.12** 评估 |

---

## 第22天 / 全量回归自动化 + 崩溃恢复 A′（2026-06-04 新增）

> **状态**：⭐ **方案 A′ + R-A** 及 **REC-A'-1～14 / REG-A'-1～5** 已用户确认（2026-06-04）；**T19.12-C** 已裁定；**专题三 RS′** 裁定点已全部确认 · **RES-IX-17** ✅；子任务 B **T-RS.0～7** ✅ 已落地（**22.11～22.12**）；**RS-CK-1～8** · **IMPL-22-RS-00～07** 主任务验收 ✅（2026-06-08）；**REC-G-03 闭合 ADR 裁定 1～4** 已用户确认（2026-06-08）· **T-REC-G03-0～4** 🟪 待子任务 B；**T-REC / T-REG** 并行项见本章。  
> **阶段**：核心稳定与商用加固（第 19～31 天 · 《38天日程》第22天）。  
> **权威前置**：第 21 天 **A′′′** ✅；**full test** [#27070111361](https://github.com/little-white1111/BlessStar/actions/runs/27070111361) ✅；第 14 天 **ATOM-XIV** ✅；第 17 天 **D 方案 / contract_gate_runner** ✅；第 19 天 **XIX-MEM-10/13** ✅。  
> **日程说明**：《38天日程》原第21天主题为「崩溃恢复」，本项目第21天已用于 **KernelPool**；**崩溃恢复**与**全量回归**在本章合并定案。

**研析参考**：BlessStar `attach_wal.c` / `attach_store.cpp` / `reload_batch_controller.cpp` / `attach_config.cpp`；`contract_gate_runner.py` / `gate_registry.json`；ZooKeeper `FileTxnSnapLog.java`（restore）；Linux journal ordered recovery；第 17 天 **XVII-TEST-ENV**。详见 **§ 研析附录**。
...
| 方案版本 | 核心思想 | 优点 | 缺点 | 状态 |
|---------|---------|------|------|------|
| **A · 冷 Re-attach** | manifest+WAL 唯一真相；重启 cold reload | 复用第14天；SIGKILL 易测 | 大 manifest 慢；RES-D-05 窗口 | ✅ **已合成入 A′ 核** |
| **B · 编排 FSM** | WAL **phase** + per_batch reconcile | 闭合 exec/persist 窗口 | WAL 扩展；裁定多 | ✅ **已合成入 A′ · P1** |
| **C · Runtime Sidecar** | `runtime.ckpt` 加速 clean restart | 秒级恢复 | ATOM 双写风险 | 📋 **A′ 可选 · P2** |
| **A′（合成）** | **A 主路径 + B per_batch + C gated** | 财务保守；可分期 | 协调成本高 | ⭐ **用户已确认采用** |

#### 专题二 · 全量回归（**R-A**）

| 方案版本 | 核心思想 | 优点 | 缺点 | 状态 |
|---------|---------|------|------|------|
| **R-A · Gate-First** | `contract_gate_runner --through-stage ci` 一键入口 | 与 D 方案 / L1 一致 | 覆盖率另脚本 | ⭐ **用户已确认采用** |
| **R-B · 回归元契约** | `C-TST-REG-1` + blocking day19 gate | 机器可读地图 | 维护成本高 | 📋 未采纳 |
| **R-C · 脚本编排** | `run_all_regression.*` 独立编排 | 灵活 | 与 lock plan 易 drift | 📋 未采纳 |

**本日裁定**：**A′ + R-A** 并行落地；**T19.12-C**（`C-TST-MEM-1` rule-only，**无** blocking smoke gate）。

#### 专题三 · Reload Session 三层复用（**RS′**）

| 方案版本 | 核心思想 | 优点 | 缺点 | 状态 |
|---------|---------|------|------|------|
| **RS-A · Controller 对象池** | checkout/checkin 多个 `ReloadBatchController` | 少 malloc | 与 kernel pool 同构误判；同 ctx **不可并行 run**；reset 易漏 | ⚠️ **研析不推荐** |
| **RS-B · 三层复用 + session reset（RS′）** | ctx/pool + store 长驻；controller **显式 reset**；测试 **ReloadSessionPolicy** | 对齐 RES-IX；生产/测试同一 API；Day19 可配 kOneShot/kReuse | 须严格 reset 契约；与 **RES-IX-5** 条文需增补 | ⭐ **用户已授权采用（2026-06-04）** |
| **RS-C · 维持 create/destroy** | 每 reload `create→run→destroy` | 状态最干净；泄漏测例简单 | Day19 smoke 重复 `store_open`/purge；与 pool 长驻模型不一致 | 📋 **kOneShot 测试策略保留** |

**触发背景**：Day19 **72h-RP** 压测与 day21 **pool warmup** 叠加后暴露——**频繁 `ReloadBatchController` create/destroy** 导致 **`BsAttachStore` 反复 open/close**（WAL purge O(n)）；与 **KernelPool 长生命周期复用** 形成架构叙事冲突。详见 **§ 专题三 RS′** 与 **AG-DAY19-STORE-1** 工程债。
...
```text
┌─────────────────────────────────────────────────────────────────────────┐
│ 第22天 · 双主线（工程上 T-REG 与 T-REC 可并行，见 § 落地建议）              │
├──────────────────────────────┬──────────────────────────────────────────┤
│ A′ 崩溃恢复                   │ R-A 全量回归                              │
│                              │                                          │
│ 崩溃/SIGKILL                  │ 开发者/CI 一键：                          │
│   → Layer-0 ATOM-XIV（已有）  │   contract_gate_runner --through-stage ci │
│   → Step-① recover_from_store │     preflight → dev_ci → ci              │
│        → RECOVERING           │   [T-REC-4 后] GATE-RECOVER              │
│   → Step-② cold_reload        │   staging（不阻塞 PR）：                    │
│        → READY                │     day19 GHA · C-TST-MEM-1 rule-only    │
│   → [per_batch] Layer-B FSM   │   collect_coverage.py（非 blocking）      │
│   → [P2] Layer-C sidecar      │                                          │
└──────────────────────────────┴──────────────────────────────────────────┘

A′ 恢复链（进程级）
  Layer-0: store_open → manifest(.prev) → wal_recover_unfinished
  Step-①: bs_adapter_attach_recover_from_store  → RECOVERING（无 IO）
  Step-②: bs_adapter_attach_recover_cold_reload → READY（IO→gate→sync + pool warmup）
  Layer-B: WAL phase · REC-A'-7 EXEC 后 → rollback 整批
  Layer-C: runtime.ckpt · 仅 clean restart · crash 永不单信 ckpt
```

**与第 14 天关系**：Layer-0 **不重复** ATOM-XIV；A′ 在 **进程重启** 边界补齐 **显式 recover + cold reload**（第14天仅 **store_open** 内 WAL recover）。

**与第 21 天关系**：Step-② 须 **kernel_pool destroy + warmup**（**KERNEL-POOL-10**）；Layer-B 对齐 **ORCH-POOL Phase B/C** exec/persist 窗口。

**与第 17 天关系**：R-A **延伸** lock plan stage 链；**不**替代 **GATE-DEV-L1 / GATE-REGRESSION**。

**与第 19 天关系**：**T19.12-C** 仅索引 day19 profile/workflow；**不**将 smoke 纳入 `--through-stage ci`。
...
| 编号 | 架构不变量 | 条文状态 | 要点 |
|------|-------------|----------|------|
| **REC-A'-1** | 持久层唯一真相 | ✅ 已确认 | 读者仅见 post-recover manifest canonical |
| **REC-A'-2** | session 作废 | ✅ 已确认 | **RES-IX-2/6**；崩溃 = 未 commit 丢失 |
| **REC-A'-3** | 不自动 merge | ✅ 已确认 | **RES-IX-12a**；recover 不 silent retry |
| **REC-A'-4** | **显式两步 API** | ✅ 已确认 | ① `recover_from_store` ② `recover_cold_reload`；**禁止** `auto_reload` |
| **REC-A'-5** | RECOVERING 拒读 | ✅ 已确认 | Step-② 成功前 **禁止** `get_snapshot` OK |
| **REC-A'-6** | scheme 边界 | ✅ 已确认 | **per_batch** 写 phase WAL；**per_path** **不**写 |
| **REC-A'-7** | EXEC 后 rollback | ✅ 已确认 | persist 未完成 → **整批 rollback** + IrSnapshot clear；**禁止** replay exec |
| **REC-A'-8** | phase 单调 | ✅ 已确认 | 对齐 **ATOM-XIV-3**；COMMIT 仅 post-flip |
| **REC-A'-9** | CM ≡ manifest | ✅ 已确认 | reconcile 逐 URI revision |
| **REC-A'-10** | B 失败降级 A | ✅ 已确认 | 全量 cold reload |
| **REC-A'-11～14** | C 层 sidecar | ✅ 已确认 | post-flip 写入；digest 绑定；feature flag；no half-pool |

#### REG-A' · 全量回归（已确认）

| 编号 | 架构不变量 | 条文状态 | 要点 |
|------|-------------|----------|------|
| **REG-A'-1** | PR blocking 入口唯一 | ✅ 已确认 | **`contract_gate_runner --through-stage ci`** |
| **REG-A'-2** | recover 进 ci 链 | ✅ 已确认 | **T-REC-4** 后注册 **GATE-RECOVER** · `-L recover;arch_gap` |
| **REG-A'-3** | day19 不阻塞 PR | ✅ 已确认 | **T19.12-C** rule-only；smoke 仍独立 GHA |
| **REG-A'-4** | 覆盖率非 blocking | ✅ 已确认 | `collect_coverage.py` 报告型 |
| **REG-A'-5** | attach 测例串行准备 | ✅ 已确认 | 本地/CI 推荐 **`run.ps1 -Prepare`** |

---

### **核心设计要点**

#### 1️⃣ recover 显式两步（REC-A'-4 · 已确认）

| 步骤 | API | 成功语义 | 失败语义 |
|------|-----|----------|----------|
| **①** | `bs_adapter_attach_recover_from_store(manifest, opts)` | 磁盘 recover OK；ctx **`RECOVERING`** | 返回 `BS_ATTACH_ERR_*` / IO；**不**进入 reload |
| **②** | `bs_adapter_attach_recover_cold_reload(ctx, opts)` | CM/runtime **READY**；revision ≡ manifest | 保持/回 **`RECOVERING`**；Report 审计 stage |

**调用方模板（财务运维）**：

```text
rc = recover_from_store(manifest, &opts);
if (rc != OK) abort;
rc = recover_cold_reload(ctx, &opts);   // 显式第二步 · 禁止合并
if (rc != OK) notify_ops();             // 磁盘 OK 但 runtime 未对齐
serve_traffic();                        // 仅 READY 后
```

#### 2️⃣ B 层 phase FSM（REC-A'-7 · 已确认）

| phase | 触发点（reload_batch） | 崩溃动作 |
|-------|------------------------|----------|
| `STAGE` | batch_begin / read 前 | orphan staging rollback |
| `GATE` | gate OK 后 | 同上 |
| **`EXEC`** | Phase B pool_submit 后 | **整批 rollback** · IrSnapshot clear |
| `PERSIST` | persist 进行中 | orphan rollback · 旧 manifest |
| `COMMIT` | manifest flip | day14 完成 flip 或 rollback |

WAL record：`PHASE_MARK { batch_epoch, phase, uri_set_hash }`（Phase 2 · **T-REC-6**）。

#### 3️⃣ R-A stage 链与一键回归

```text
preflight   GATE-TEST-HERM / TIER / POL
dev_ci      GATE-DEV-L1  (-L day17)
ci          GATE-INTEGRATION (-j 1) → GATE-REGRESSION (-LE day17 -j 1)
recover     GATE-RECOVER (-L recover;arch_gap)     ← T-REG-3 · 依赖 T-REC-4
staging     day19-smoke GHA · C-TST-MEM-1 索引      ← 不进入 --through-stage ci
```

**本地一键（PR 前）**：

```text
tools/test/run.ps1 -Prepare -BuildDir build_ci_test -Config Release
python tools/scripts/contracts/contract_gate_runner.py --through-stage ci
python tools/scripts/test/collect_coverage.py --build-dir build_ci_test   # 可选
```

#### 4️⃣ T19.12-C · C-TST-MEM-1 rule-only

| 字段 | 约定 |
|------|------|
| `rule` | 引用 **XIX-MEM-10/13**、**`day19_profile.json`**、`.github/workflows/day19-stress*.yml` |
| `gate_refs` | **空** 或仅文档型 staging 引用（**非** blocking） |
| `implementations[]` | 指向现有 ctest 标签 + 脚本路径（**sync** 时只读校验存在性） |
| 执行 | **`ctest -L day19;stress`** + 既有 GHA；**不**改 lock plan ci stage |

#### 5️⃣ arch_gap 测例矩阵（`.cursor/rules/architecture-shortcoming-iteration.mdc`）

| ID | 测例 | LABELS |
|----|------|--------|
| **AG-REC-COLD-1** | `AttachRecoverColdTest` | `recover;arch_gap;regression` |
| **AG-REC-TWO-STEP-1** | 同上 · 仅 Step-① | 同上 |
| **AG-REC-EXEC-ROLLBACK-1** | `AttachRecoverFsmTest` | Phase 2 |
| **AG-REC-CONC-1** | `AttachRecoverConcurrencyTest` | `recover;arch_gap` |
| **AG-REC-SCHEME-1** | per_path 无 PHASE WAL | Phase 2 |
| **AG-REC-SIDE-1** | `AttachRecoverSidecarTest` | Phase 3 |
...
| 选择项 | 用户选择 | 说明 |
|--------|---------|------|
| 崩溃恢复合成 | **A′** | A 核 + B per_batch + C 可选 |
| **REC-A'-7** | **rollback 整批** | EXEC 后 persist 前崩溃；**禁止** MVP replay exec |
| recover API | **显式两步** | **禁止** `auto_reload` |
| 全量回归 | **R-A** | Gate-First · `--through-stage ci` |
| **T19.12** | **T19.12-C** | `C-TST-MEM-1` rule-only；**无** blocking smoke gate |
| 入库授权 | **已授权** | 本章写入 `架构方案选择记录.md` § 第22天 |

---

### **落地建议（子任务B待执行 · T-REC + T-REG 最终裁定版）**

> **推荐总顺序（强制）**：**T-REG-0/1（文档+rule-only 契约）→ T-REC-0～3（两步 API + RECOVERING）→ T-REC-4/5（测例+文档）→ T-REG-3（GATE-RECOVER）→ T-REG-2/4 → [P1] T-REC-6～10 → [P2] T-REC-11～13**。  
> **前置**：第 14 天 **ATOM-XIV** ✅；第 21 天 **KernelPool** ✅；第 17 天 **contract_compile** 可用。  
> **禁止**：未实现 Step-② 即注册 **GATE-RECOVER**；**禁止** MVP `auto_reload`；**禁止** blocking day19 gate（T19.12-C）。
...
#### 阶段 T-REG-0 — 回归文档与索引（R-A · 可与 T-REC 并行）

| 序 | IMPL | 落地项 | 状态 | 影响范围 / 交付物 |
|----|------|--------|------|-------------------|
| T-REG.0 | **IMPL-22-REG-00** | `docs/DAY22_REGRESSION_CHAIN.md` | ✅ | stage 链 · 本地/CI 命令 · 与 A′ 衔接 |
| T-REG.1 | **IMPL-22-REG-01** | **`C-TST-MEM-1.v1.json`** rule-only（**T19.12-C**） | ✅ | `docs/contracts/testing/`；`contract_validate`；**无**新 blocking gate |
| T-REG.2 | **IMPL-22-REG-02** | `tools/scripts/test/collect_coverage.py` | ✅ | 汇总 ctest 标签覆盖；**REG-A'-4** |
| T-REG.4 | **IMPL-22-REG-03** | README / `docs/CI_LOOP_WORKFLOW.md` 同步 | ✅ | 一键回归入口 |

**T-REG-0 实现要点**

1. **DAY22_REGRESSION_CHAIN.md** 须含：**R-A stage 表**、Windows `run.ps1 -Prepare`、Linux 等价脚本路径、**staging vs ci** 边界。  
2. **C-TST-MEM-1** 的 `rule` 引用 **XIX-MEM-10/13** 阈值名与 **`tools/scripts/perf/day19_profile.json`** 键路径；**`gate_refs`: []** 或 staging 非 blocking。  
3. **collect_coverage.py**：扫描 `cmake/Tests.cmake` LABELS；输出 JSON/Markdown；**不得** fail PR。  
4. **契约**：仅 **T-REG-1** 触达 `docs/contracts/**`；**不**改 `contract_plan.lock.json` ci blocking 集合（除非 **T-REG-3** 增 GATE-RECOVER）。

#### 阶段 T-REC-0 — 两步 recover API（A′ Phase 1 · P0）

| 序 | IMPL | 落地项 | 状态 | 影响范围 / 交付物 |
|----|------|--------|------|-------------------|
| T-REC.0 | **IMPL-22-REC-00** | `attach_recover.h` + Step-① 实现 | ✅ | `adapter/include/bs/adapter/attach_recover.h`；`adapter/src/attach_recover.cpp` |
| T-REC.1 | **IMPL-22-REC-01** | `RECOVERING` 状态 + API 守卫 | ✅ | `attach_session.*`；`attach_config.cpp` **REC-A'-5** |
| T-REC.2 | **IMPL-22-REC-02** | Step-② `recover_cold_reload` | ✅ | manifest URI 枚举 · IO→gate→sync |
| T-REC.3 | **IMPL-22-REC-03** | pool destroy + warmup on recover | ✅ | `attach_context.cpp` · **KERNEL-POOL-10** |

**T-REC-0 实现要点（子任务 B 必读 · Step-①）**

1. **不**在 `store_open` 内 silent full reload；Step-① 仅包装现有 **`bs_adapter_attach_persist_store_open` + wal_recover** + attach 会话创建。  
2. 成功后 **`bs_adapter_attach_session_set_recovering(ctx, 1)`**；所有读 API 返回 **`BS_ATTACH_ERR_RECOVERING`**（或等价）。  
3. **禁止** `opts.auto_reload`（MVP）；若 struct 预留字段，文档标 **刻意未定义**。  
4. watch 事件：可选 `BS_ATTACH_WATCH_STAGE_RECOVER_BEGIN`（对齐 **WATCH-XV-6** 模式）。  
5. **INC-VIII-2**：recover 实现 **不得** kernel 层 include `ConfigManager.h`。

**T-REC-0 实现要点（子任务 B 必读 · Step-②）**

1. 枚举 manifest URI（store API 或新 **`bs_adapter_attach_persist_store_foreach_uri`**）。  
2. 对每个 URI：**IoFacade.read → reload_gate → sync_path**（per_path）或 batch cold 路径（per_batch 无 B 时全量）。  
3. 开始前 **`bs_kernel_pool_destroy` + warmup`**（**REC-A'-14** 预置）；禁止 half-pool。  
4. 全部成功 → **`RECOVERING=0` / READY**；任一失败 → 保持 RECOVERING + Report audit。  
5. 与 **XX-CONC-5** 一致：recover 路径 **单线程**（测例可串行；**AG-REC-CONC-1** 验证拒读）。

#### 阶段 T-REC-1 — 测例 / gate / 文档（A′ Phase 1 · P0）

| 序 | IMPL | 落地项 | 状态 | 影响范围 / 交付物 |
|----|------|--------|------|-------------------|
| T-REC.4 | **IMPL-22-REC-04** | `AttachRecoverColdTest` + arch_gap | ✅ | `AG-REC-COLD-1` / `AG-REC-TWO-STEP-1`；Linux subprocess SIGKILL |
| T-REC.5 | **IMPL-22-REC-05** | `docs/DAY21_CRASH_RECOVERY.md` | ✅ | A′ 专题运维说明 |
| T-REG.3 | **IMPL-22-REG-04** | **GATE-RECOVER** + `cmake/Tests.cmake` | ✅ | `gate_registry.json`；`-L recover`；**依赖 T-REC.4** |

**T-REC-1 实现要点**

1. **AttachRecoverColdTest**：fork 子进程 → 注入 kill 点（post-WAL-pre-flip / mid-reload）→ **SIGKILL** → 父进程 Step-①② → 断言 manifest revision。  
2. **Windows**：无 SIGKILL 时用 **mock inject** + Step-①② 单元路径；Linux CI 为 **权威** subprocess 测例。  
3. **cmake**：`bs_test_attach_recover_cold` · `LABELS recover;arch_gap;regression` · **`RESOURCE_LOCK attach_integration`**。  
4. **GATE-RECOVER**：`ctest ... -L recover -j 1`；stage=**ci**；**T-REC.4 PASS 后**才注册 lock plan。  
5. 契约改 gate 后：**`contract_validate` + `contract_compile` + `sync_contract_implementations.py`**。

#### 阶段 T-REC-2 — B 层 FSM（A′ Phase 2 · P1 · per_batch）

| 序 | IMPL | 落地项 | 状态 | 影响范围 / 交付物 |
|----|------|--------|------|-------------------|
| T-REC.6 | **IMPL-22-REC-06** | WAL `PHASE_MARK` record | ✅ | `attach_wal.c/h` · `BsAttachWalRecoverPhase` |
| T-REC.7 | **IMPL-22-REC-07** | `reload_batch_controller` phase hook | ✅ | **REC-A'-7** rollback · `testing_set_abort_after_exec` |
| T-REC.8 | **IMPL-22-REC-08** | CM reconcile vs manifest | ✅ | `reconcile_cm_with_manifest` · **REC-A'-9** |
| T-REC.9 | **IMPL-22-REC-09** | `AttachRecoverFsmTest` | ✅ | **AG-REC-EXEC-ROLLBACK-1** PASS |
| T-REC.10 | **IMPL-22-REC-10** | B→A 降级 + `AG-REC-SCHEME-1` | ✅ | per_path 无 PHASE · `wal_count_record_type(5)==0` |

**T-REC-2 实现要点**

1. **PHASE_MARK** 仅 **`attach_scheme==per_batch`** 写入；**REC-A'-6** 单测 guard。  
2. **EXEC 后 kill 测例**：断言 **manifest epoch 不变**、IrSnapshot **空**、CM **未** authoritative sync 新 rev。  
3. B reconcile 失败 → 调用 Step-② 全量 cold（**REC-A'-10**）。  
4. **ATOM-REC-SAFE-2**：phase WAL corrupt → 保守；**不**删 canonical。

#### 阶段 T-REC-3 — C sidecar（A′ Phase 3 · P2 · 刻意延后）

| 序 | IMPL | 落地项 | 状态 | 影响范围 / 交付物 |
|----|------|--------|------|-------------------|
| T-REC.11～13 | **IMPL-22-REC-11** | `runtime.ckpt` + fast hydrate | ✅ | **REC-A'-11～14** · **AG-REC-SIDE-1** |

**T-REC-3 实现要点**

1. Sidecar 路径 `{manifest}.runtime.ckpt`；**REC-A'-12** manifest/URI payload digest 绑定。  
2. **REC-A'-11** 仅 Step-② READY 后写入 `CLEAN_SHUTDOWN`；`reload_batch_run` 开头 invalidate。  
3. **REC-A'-13** 默认 off；`BS_ATTACH_RECOVER_SIDECAR=1` 启用；crash/WAL exec rollback **拒绝** sidecar。  
4. fast hydrate 失败 → 无损降级 cold reload（**REC-A'-10**）。  
5. **AG-REC-SIDE-1** `AttachRecoverSidecarTest` · `-L recover;arch_gap`。

#### 内核层 / 适配层 / 工具新增（路径索引）

| 文件 / 目录 | 说明 |
|-------------|------|
| `adapter/include/bs/adapter/attach_recover.h` | 两步 API 公开面 |
| `adapter/src/attach_recover.cpp` | Step-①② 实现 |
| `adapter/test/AttachRecoverColdTest.cpp` | **T-REC.4** |
| `adapter/test/AttachRecoverSidecarTest.cpp` | **T-REC.11～13**（Phase 3） |
| `adapter/include/bs/adapter/attach_recover_sidecar.h` | Layer-C API |
| `adapter/persistence/attach_runtime_sidecar.c` | `runtime.ckpt` format |
| `docs/DAY21_CRASH_RECOVERY.md` | A′ 运维 |
| `docs/DAY22_REGRESSION_CHAIN.md` | R-A 一键回归 |
| `docs/contracts/testing/C-TST-MEM-1.v1.json` | **T19.12-C** |
| `tools/scripts/test/collect_coverage.py` | **T-REG.2** |
| `docs/gates/gate_registry.json` | **GATE-RECOVER** |
| `cmake/Tests.cmake` | `-L recover;arch_gap` |

#### 验收标准（主任务审核用）

| 项 | 标准 |
|----|------|
| **REC-A'-4** | 存在两步 API；**无** `auto_reload`；**AG-REC-TWO-STEP-1** PASS |
| **REC-A'-7** | **AG-REC-EXEC-ROLLBACK-1** PASS（Phase 2） |
| **REC-A'-11～14** | **AG-REC-SIDE-1** PASS（Phase 3 · flag gated） |
| **REG-A'-1** | `contract_gate_runner --through-stage ci` 文档化且 CI 绿 |
| **T19.12-C** | **`C-TST-MEM-1`** validate OK；**无** 新 blocking smoke gate |
| Phase 1 | `-L recover` PASS；`-L day14` **2/2**；`-L regression` 无退化 |
| 契约 | 改 gate/contract 后：**validate + compile + sync implementations** |
| 台账 | `项目修改记录.md` 第 **22** 天变更 |
...
| 问题 | 解决方式 |
|------|---------|
| 进程崩溃后配置视图不确定 | **A′** Layer-0 + 显式两步 recover |
| pool **EXEC** 后 **persist** 前窗口（第21天） | **REC-A'-7** rollback 整批 + Phase 2 WAL |
| **RES-D-05** 磁盘 vs 运行时 | **§ REC-G-03 闭合** · post-sync + WIRE + Step-② |
| 测例分散、无 PR 一键入口 | **R-A** `contract_gate_runner --through-stage ci` |
| **T19.12** 契约空缺 | **T19.12-C** rule-only 索引 |
| recover 未进 regression | **GATE-RECOVER**（**T-REG.3**） |
| 第38天日程「崩溃恢复」未落地 | 本章 **A′** + **T-REC** |
| 第38天日程「全量回归串联」 | 本章 **R-A** + **T-REG** |

---

### **方案状态**

- **方案确认时间**：2026-06-04（用户确认 **A′ + R-A + T19.12-C + REC-A'-7 + 显式两步**）  
- **子任务 A**：✅ 研析 + 模板整理 + 三轮架构短板迭代定稿  
- **子任务 B**：✅ **Phase 1～3（T-REC.0～13 + T-REG.0～4）** 已落地  
- **状态**：🟡 **待主任务审核**（第22天 A′ + R-A 全阶段完工）

#### 子任务B 落地状态总结（第22天）

| 类别 | 内容 |
|------|------|
| 当前阶段 | **Phase 3（P2 · Layer-C）** ✅ 已完成 |
| 落地项 | **T-REG.0～4** ✅ + **T-REC.0～13** ✅ |
| 实现文件 | Phase 1～2 见 **§ 路径索引** + `attach_runtime_sidecar` · `attach_recover_sidecar` |
| 测试 | `bs_test_attach_recover_{cold,fsm,sidecar,concurrency}` · `-L recover` ✅ **4/4** |
| 验证结果 | ✅ `ctest -L recover -j 1` 3/3；✅ **AG-REC-SIDE-1** |
| 台账 | ✅ **`项目修改记录.md` 第22天** 22.1～22.8 |
...
| 未定义项 | 说明 | 第22天动作 |
|----------|------|------------|
| **`auto_reload`** | 一步 recover+reload | **禁止**（REC-A'-4） |
| **replay exec**（EXEC 后） | 幂等重放 IR exec | **禁止**（REC-A'-7）；另日决策 |
| **GATE-TEST-DAY19-SMOKE** blocking | PR 阻塞 smoke | **禁止**（T19.12-C） |
| **C-TST-REG-1** 元契约 | R-B 全量绑定 | **不建**（R-A） |
| **Layer-C sidecar** | fast restart | **P2** · **默认 off** · `BS_ATTACH_RECOVER_SIDECAR=1` |
| **Linux 72h full** | L3 长稳 | 继承第19天遗留 |
| **多进程 manifest 写** | **RES-D-03** | 部署 ADR · 非第22天 |
...
| ID | 短板主题 | 架构影响 | 建议补齐时机 | 状态 | 参见 |
|----|-----------|----------|--------------|------|------|
| **REC-G-01** | 无进程 recover API | 崩溃后不可验收 | **T-REC.0** | ✅ | **REC-A'-4** |
| **REC-G-02** | exec/persist 窗口 | 月结半批风险 | **T-REC.6～7** | ✅ | **REC-A'-7** · **AG-REC-EXEC-ROLLBACK-1** |
| **REC-G-03** | **RES-D-05** 双真相 | 错账窗口 | **T-REC-G03** | 🟡 | **§ REC-G-03 闭合** · ADR ✅ 待 B |
| **REC-O-01** | RECOVERING 并发读 | 脏读 | **AG-REC-CONC-1** | ✅ | **REC-A'-5** · `AttachRecoverConcurrencyTest` |
| **REC-O-02** | sidecar 顺序 | ATOM 风险 | Phase 3 | ✅ | **REC-A'-11** · invalidate on reload |
| **RS-G-01** | controller/store 生命周期分裂 | Day19 I/O 放大 | **T-RS.1** | ✅ | **挂点 A** · **RES-IX-17** |
| **RS-G-02** | 无公开 `reload_batch_reset` | 无法多轮复用 | **T-RS.0～2** | ✅ | **IMPL-22-RS-00** · **RES-IX-17** |
| **REG-G-01** | 无一键回归文档 | 运维分散 | **T-REG.0** | ✅ | **REG-A'-1** |
| **REG-G-02** | 无覆盖率汇总 | 覆盖不可见 | **T-REG.2** | ✅ | **REG-A'-4** |
| **REG-G-03** | T19.12 契约 | 跨日索引 | **T-REG.1** | ✅ | **T19.12-C** 已裁定 |
| **REG-G-04** | recover 未入链 | 回归缺口 | **T-REG.3** | ✅ | **REG-A'-2** |
| **RS-G-01** | controller 绑 store 生命周期 | Day19 重复 open/purge | **T-RS.1～7** | ✅ | **ctx persist_store** · **AG-RS-STORE-1** |
| **RS-G-02** | 无公开 reset API | 生产只能 destroy | **T-RS.0～2** | ✅ | **AG-RS-RESET-1** · **RES-IX-17** |

---

### **短板登记维护约定**

- 本日表用 **REC-G-xx / REG-G-xx / REC-O-xx**；闭合后同步 **§ 架构短板跨日总索引**。  
- **REG-G-03** 裁定 ✅ 后，子任务 B 完成 **T-REG.1** 时标 ✅。  
- **REC-G-02** 依赖第21天 **ORCH-POOL Phase B/C** 证据锚点。  
- **REC-G-03**：ADR 裁定 1～4 已入库（2026-06-08）；**T-REC-G03-0～4** 全部 PASS 后标 ✅ 并同步 **RES-D-05** / **RES-G-04**。

---

### **REC-G-03 闭合 · RES-D-05 双真相 + WIRE/state 同步（2026-06-08 用户裁定入库）**

> **状态**：⭐ **ADR 裁定 1～4 已用户确认**（2026-06-08）；子任务 B **T-REC-G03-0～4** 🟪 待执行；工程全部 PASS 后 **REC-G-03** / **RES-D-05** / **RES-G-04** 标 ✅。  
> **前置**：第 12 天 **RES-IX-3**；第 17 天 **XVII-CM-1～4** / **XVII-KERNEL-4**；第 20 天 **XX-CONC-1～3**；第 22 天 **T-REC.2/8**（Step-② + `reconcile_cm_with_manifest`）✅。  
> **关联**：**RES-G-04**（读者绑持久层）、**R8-14** / **WIRE-07-01～03**、**IMPL-08-23/24**。

#### **REC-G-03 闭合 · 用户裁定（ADR 1～4）**

##### 裁定 1 · 触发点契约（按路径类型）

| 契约 ID | 路径类型 | 允许「只 commit 不同步」 | 产品强制路径 | 违约处置 |
|---------|----------|--------------------------|--------------|----------|
| **C-ATTACH-SYNC-1** | 测试 / 单测 store | ✅（隔离测） | 仅限 `adapter/test/**`、`-L arch_gap` 等 | 文档 + 静态守卫：生产树禁止直调 `persist_store_commit*` |
| **C-ATTACH-SYNC-2** | **产品写配置** | ❌ | **仅** `reload_batch_run` 或 recover **Step-②** `recover_cold_reload` | **GATE-ATTACH-SYNC-PROD** |
| **C-ATTACH-SYNC-3** | recover `fast_hydrate` | ✅（无 commit） | canonical 读字节 → `config_sync` → **`reconcile_cm_with_manifest`** | `-L recover` |
| **C-ATTACH-SYNC-4** | recover Step-② cold | ✅（reload 内 commit） | 全链 `reload_batch_run` | **AG-REC-COLD-1** |
| **C-ATTACH-SYNC-5** | **外部改盘**（RES-D-02） | ❌ | 检测 revision/hash → **拒读**或 **强制 cold reload** | 运维 ADR + 可选 watchdog |
| **C-ATTACH-SYNC-6** | 只 `config_sync` 不 commit | ❌（产品） | 测试专用 | 同 **C-ATTACH-SYNC-1** |

**铁则（C-ATTACH-SYNC-2）**：`adapter/src`、CLI、非 test 入口 **禁止** 直接调用 `bs_adapter_attach_persist_store_commit_*` / `batch_commit`；persist 仅允许出现在 `reload_batch_controller` 与 recover 编排内。

##### 裁定 2 · 权威顺序

- **manifest revision 唯一权威**（RES-IX-3）。  
- CM / `AttachSession.path_revision` **必须与 manifest revision 单调对齐**；`reconcile_cm_with_manifest` 不一致 → **fail-fast**（`BS_ATTACH_ERR_CONFLICT`），禁止静默双真相。  
- recover / 正常运行 / 外部改盘检测均以此为准。

##### 裁定 3 · 执行态（Kernel + IrSnapshot）

| 编号 | 不变量 | 要点 |
|------|--------|------|
| **REC-G03-3a** | `config_sync` 后 Kernel 清空 | 每次 `bs_adapter_attach_config_sync_path` **成功**后，对 active ctx **全部** Kernel/Pool slot 执行 **`bs_pipeline_reset`**（方案 C）；**不**在 sync 内自动 re-exec |
| **REC-G03-3b** | IrSnapshot 消耗即销毁 | `publish` → `pin` → `exec` → `unpin` → **立即 destroy entry**；与 `config_sync` **无关**；禁止跨 reload 保留 latest entry |
| **REC-G03-3c** | 主链顺序不变 | **XVII-KERNEL-4**：gate → ir_execute → persist → config_sync；**3a** 在 sync **之后** |

##### 裁定 4 · Notify（弱一致 + 写窗口 flush）

| 编号 | 不变量 | 要点 |
|------|--------|------|
| **REC-G03-4a** | 弱一致 | 真相以 StateBus + revision 为准（**XX-CONC-1**）；允许 notify lag；禁止 revision 回退 |
| **REC-G03-4b** | 完成边界 | **最外层** `end_write_window` 后：`drain_deferred_events` + `notify_queue_flush`；`reload_batch_run` 成功返回前必须满足 |
| **REC-G03-4c** | 通道分裂 | **attach_watch**（persist）单次 `publish` 同步完成，**不**纳入 CM 写窗口 flush 统一完成点 |
| **REC-G03-4d** | 非闭合条件 | REC-G-03 **不**以 listener 业务副作用完成作为闭合条件 |

**刻意不纳入 MVP**：notify 独立 QPS 限流；`wait_notify` 作为产品默认强一致（API 已存在，二期选用）。

#### **方案解决的核心问题（闭合后）**

| 问题 | 解决方式 |
|------|---------|
| 产品路径绕过 reload 写盘 | **C-ATTACH-SYNC-2** + **GATE-ATTACH-SYNC-PROD** |
| manifest vs CM 双真相 | 裁定 2 + post-sync revision 绑定 + **RES-G-04** |
| 热更后 IrSnapshot 过期 IR | **REC-G03-3b** 消耗即销毁 |
| 热更后 Kernel stage 脏态 | **REC-G03-3a** sync 后 pipeline reset |
| 入口层 WIRE 缺口 | **WIRE-07-01/03** · **T-REC-G03-2** |
| notify 与双真相混淆 | 裁定 4：notify 旁路观测，**非**真相源 |

#### **REC-G-03 闭合验收矩阵**

| ID | PASS 条件 | 测例标签 |
|----|-----------|----------|
| **AG-DUAL-TRUTH-1** | 产品树无裸 `persist_store_commit` | `arch_gap` + gate |
| **AG-DUAL-TRUTH-2** | reload 后 CM revision == manifest revision | `arch_gap;regression` |
| **AG-DUAL-TRUTH-3** | 注入「persist 成功跳过 sync」必 fail | `arch_gap` |
| **AG-IR-SNAP-1** | exec 成功后 `ir_snapshot_entry_count==0` | `arch_gap` |
| **AG-KERNEL-RESET-1** | `config_sync` 后全 slot pipeline stage == IDLE | `arch_gap;runtime` |
| **AG-NOTIFY-FLUSH-1** | 写窗口关闭后 EventBus pending 清空且 watch 队列空 | `arch_gap` |
| **AG-WIRE-FULL-1** | 入口全链 commit 后 revision 对齐 | `wire;integration` |
| **AG-READER-REV-1** | meta.revision == manifest；chunk 跨 reload handle 失效 | `arch_gap` |
| **AG-READER-REV-2** | 注入 manifest 变更跳过 sync → 读 API stale | `arch_gap` |

**闭合条件**：上表 + `-L recover` 无退化 + **C-ATTACH-SYNC-*** validate/compile → **REC-G-03** / **RES-D-05** / **RES-G-04** 标 ✅。

---

### **落地建议（子任务 B · T-REC-G03 · 详细）**

> **推荐总顺序（强制）**：**T-REC-G03-0（契约+守卫）→ T-REC-G03-1（post-sync 钩子）→ T-REC-G03-2（WIRE + IrSnapshot）→ T-REC-G03-3（notify 验收）→ T-REC-G03-4（RES-G-04 读者）**  
> **禁止**：产品路径新增裸 persist；IrSnapshot 跨 reload 保留；`config_sync` 内自动 re-exec。
...
#### 阶段 T-REC-G03-0 — 触发点契约 + 静态守卫（裁定 1 · P0）

| 序 | IMPL | 落地项 | 状态 | 影响范围 / 交付物 |
|----|------|--------|------|-------------------|
| T-REC-G03.0 | **IMPL-REC-G03-00** | **C-ATTACH-SYNC-1～6** 契约实例 | 🟪 | `docs/contracts/attach/` · `gate_refs` |
| T-REC-G03.1 | **IMPL-REC-G03-00b** | **GATE-ATTACH-SYNC-PROD** | 🟪 | `tools/scripts/gates/check_attach_sync_prod_paths.py`；`gate_registry.json` |

**T-REC-G03-0 实现要点**

1. **check_attach_sync_prod_paths.py**：扫描 `adapter/src`、CLI 路径，禁止 `persist_store_commit` / `batch_commit`（白名单：`reload_batch_controller.cpp`、`attach_store.cpp` 实现体、`adapter/test/**`）。  
2. 契约 **C-ATTACH-SYNC-2** `rule` 写明产品仅 `reload_batch_run` / recover Step-②。  
3. 触达契约后：**`contract_validate_instances.py` + `contract_compile.py` + `sync_contract_implementations.py`**。

#### 阶段 T-REC-G03-1 — 统一 post-sync 钩子（裁定 2 + 3a · P0）

| 序 | IMPL | 落地项 | 状态 | 影响范围 / 交付物 |
|----|------|--------|------|-------------------|
| T-REC-G03.2 | **IMPL-REC-G03-01** | `bs_adapter_attach_post_config_sync` | 🟪 | `attach_config.h` / `attach_config.cpp` |
| T-REC-G03.3 | **IMPL-REC-G03-01b** | `bs_adapter_attach_kernel_reset_all_pipelines` | 🟪 | `attach_context.cpp` 或 `attach_execute.cpp` |
| T-REC-G03.4 | **IMPL-REC-G03-01c** | `bs_adapter_attach_session_set_path_revision` | 🟪 | `attach_session.*`（manifest 权威，替代单纯 bump） |

**T-REC-G03-1 实现要点**

1. **审计结论（当前）**：产品 persist 仅 `reload_batch_controller.cpp`；`fast_hydrate` 仅 `config_sync` 无 commit。所有成功 `config_sync` 路径须调用 **同一 post-sync 钩子**。  
2. **钩子顺序（固定）**：

```text
config_sync_path == 0
  → post_config_sync(ctx, uri, store)
       ① persist_store_get_revision → manifest_rev
       ② session_set_path_revision(uri, manifest_rev)
       ③ kernel_reset_all_pipelines(ctx)   // 全 slot bs_pipeline_reset
       ④ （debug）单 path reconcile 断言
```

3. **kernel_reset_all_pipelines**：单 Kernel RUNNING → reset default pipeline；Pool warmed → 每 slot reset；**禁止** destroy pool / **禁止** re-exec。  
4. **reload 末**保留 `reconcile_cm_with_manifest` 全量对账。  
5. **测例**：**AG-DUAL-TRUTH-2**、**AG-KERNEL-RESET-1**。

#### 阶段 T-REC-G03-2 — WIRE 全链 + IrSnapshot 消耗即销毁（裁定 3b · P1）

| 序 | IMPL | 落地项 | 状态 | 影响范围 / 交付物 |
|----|------|--------|------|-------------------|
| T-REC-G03.5 | **IMPL-REC-G03-02** | `exec_path_ir` 成功后 `ir_snapshot_remove` | 🟪 | `reload_batch_controller.cpp` · `attach_ir_snapshot.cpp` |
| T-REC-G03.6 | **IMPL-REC-G03-02b** | **WIRE-07-01** CLI attach→reload 守卫 | 🟪 | 入口层 · **IMPL-08-23** |
| T-REC-G03.7 | **IMPL-REC-G03-02c** | **WIRE-07-03** + **AG-WIRE-FULL-1** | 🟪 | `AttachWireFullChainIntegrationTest.cpp` · `-L wire;integration` |
| T-REC-G03.8 | **IMPL-REC-G03-02d** | **C-ATTACH-WIRE-1** 契约（可选） | 🟪 | `docs/contracts/attach/` |

**T-REC-G03-2 实现要点**

1. **IrSnapshot**：`unpin` 后立即 `bs_adapter_attach_ir_snapshot_remove(handle)`；失败路径仍 `clear_all`。  
2. **全链形状**：`freeze → reload_batch_run → read → gate → [publish→exec→destroy] → persist → config_sync → post_sync_hook`。  
3. **AG-WIRE-FULL-1**：`RESOURCE_LOCK attach_integration`；断言 revision 对齐 + `ir_snapshot_entry_count==0`。  
4. **WIRE-07-02**（log level）不阻塞 REC-G-03 闭合。

#### 阶段 T-REC-G03-3 — Notify 弱一致验收（裁定 4 · P1）

| 序 | IMPL | 落地项 | 状态 | 影响范围 / 交付物 |
|----|------|--------|------|-------------------|
| T-REC-G03.9 | **IMPL-REC-G03-03** | **AG-NOTIFY-FLUSH-1** 测例 | 🟪 | `AttachNotifyFlushTest.cpp` |
| T-REC-G03.10 | **IMPL-REC-G03-03b** | `DAY20_CONCURRENCY_MODEL.md` 增补裁定 4 条文 | 🟪 | docs |

**T-REC-G03-3 实现要点**

1. **现状已具备**：最外层 `end_write_window` → `drain_deferred_events` + `notify_queue_flush`（`attach_session.cpp`）。本阶段以 **测例固化** 为主，**不**新增 QPS 限流。  
2. **AG-NOTIFY-FLUSH-1**：写窗口内 listener 未触发；`reload_batch_run` 返回后 EventBus pending 空 + watch 队列空。  
3. Pool exec 临时关窗 **不**作为产品级 notify 完成点（仅最外层关窗）。  
4. **attach_watch**（persist）保持单次 `publish` 同步语义，与 CM flush **分列**（**REC-G03-4c**）。

#### 阶段 T-REC-G03-4 — RES-G-04 读者 revision 绑定（裁定 2 读侧 · P0）

| 序 | IMPL | 落地项 | 状态 | 影响范围 / 交付物 |
|----|------|--------|------|-------------------|
| T-REC-G03.11 | **IMPL-REC-G03-04** | 读 API 绑 manifest revision | 🟪 | `attach_session.cpp` · `get_snapshot_meta` / `open_snapshot_read` |
| T-REC-G03.12 | **IMPL-REC-G03-04b** | 外部改盘 stale 检测（**C-ATTACH-SYNC-5**） | 🟪 | 读前 `manifest_rev != session_rev` → `BS_ATTACH_ERR_REVISION_STALE` |
| T-REC-G03.13 | **IMPL-REC-G03-04c** | **AG-READER-REV-1/2** | 🟪 | `adapter/test/` · `-L arch_gap` |

**T-REC-G03-4 实现要点**

1. **被动策略 A**：写路径保证 sync 后 revision 对齐；读者 **不**触发 lazy reload（**C-ATTACH-SYNC-2**）。  
2. `open_snapshot_read` / `read_chunk` 已有 `REVISION_CHANGED`；补 **meta 与 manifest 不一致** 拒读。  
3. 可选强一致：已有 `read_since_meta` / `wait_notify` — **非** MVP 默认（**REC-G03-4a**）。  
4. 闭合后 **RES-G-04** 跨日表标 ✅。
...
| 路径 | 职责 |
|------|------|
| `adapter/include/bs/adapter/attach_config.h` | `post_config_sync` 公开面 |
| `adapter/src/attach_config.cpp` | post-sync 钩子；`fast_hydrate` 接入 |
| `adapter/orchestration/reload_batch_controller.cpp` | persist 后调 post-sync；IrSnapshot remove |
| `adapter/src/attach_ir_snapshot.cpp` | `ir_snapshot_remove` |
| `adapter/src/attach_session.cpp` | `set_path_revision`；读者 stale |
| `tools/scripts/gates/check_attach_sync_prod_paths.py` | **GATE-ATTACH-SYNC-PROD** |
| `adapter/test/Attach*DualTruth*` / `AttachWireFullChain*` / `AttachNotifyFlush*` | arch_gap / wire 测例 |

#### T-REC-G03 轮次元信息

| 字段 | 值 |
|------|-----|
| 轮次 | **REC-G-03 闭合**（RES-D-05 + WIRE/state 同步） |
| 用户已裁决 | 裁定 **1～4**（2026-06-08） |
| 子任务 B | **T-REC-G03-0～4** 🟪 待执行 |
| 闭合后 | **REC-G-03**、**RES-D-05**、**RES-G-04** → ✅ |
...
| 来源 | 路径或符号 |
|------|------------|
| WAL recover | `adapter/persistence/attach_wal.c` `bs_adapter_attach_persist_wal_recover_unfinished` |
| store open | `adapter/persistence/attach_store.cpp` |
| reload 编排 | `reload_batch_controller.cpp` Phase A/B/C |
| gate runner | `tools/scripts/contracts/contract_gate_runner.py` `--through-stage` |
| regression gate | `docs/gates/gate_registry.json` **GATE-REGRESSION** |
| ZK restore | `FileTxnSnapLog.java` |
| 架构迭代规则 | `.cursor/rules/architecture-shortcoming-iteration.mdc` |

#### 结构化对比表（A′ 崩溃恢复）

| 对比维度 | 单 A | 单 B | 单 C | **A′（已定）** |
|----------|------|------|------|----------------|
| MVP 可交付 | ✅ | ❌ | ❌ | ✅ Phase 1 |
| REC-A'-7 | N/A | ✅ | N/A | ✅ rollback |
| recover API | 模糊 | 模糊 | N/A | ✅ 显式两步 |
| 证据 | `attach_wal_recover` | `reload_batch_controller` | `checkpoint_path` | 本章 |
...
| 轮次 | 要点 |
|------|------|
| 1 | 识别 **REC-G / REG-G / REC-O** |
| 2 | **REC-A'-4** 两步 + **REC-A'-7** rollback + C gated |
| 3 | **arch_gap** 测例矩阵 → **T-REC.4/9/10** |

---

### **Reload Session 三层复用架构（第22天 · RS′ · 专题三 · 2026-06-04）**

**用户授权摘要（2026-06-04）**：采纳 **RS′** 并闭合全部裁定点 — **挂点 A**（`AttachContext::persist_store`）；主 API **`reload_batch_reset`**；**RES-IX-17** ✅；smoke **day = kReuseController**、**night = kBatchNight**、**arch_gap = kOneShot**；**XIX-MEM-10** 不变；**iter = run 次数**。

**研析参考**：`reload_batch_controller.cpp` `destroy`/`ensure_attach_store`；`attach_store.cpp` `store_open`/purge；`attach_context.cpp` pool warmup；`Day19StressReloadLoopTest.cpp`；**RES-IX-5**；**XIX-MEM-8**；**DAY20_CONCURRENCY_MODEL.md** § 写窗口。

#### 方案演进（专题三 · RS′）

| 方案 | 核心思想 | 优点 | 缺点 | 状态 |
|------|----------|------|------|------|
| **RS-A** | Controller **对象池**（checkout/checkin） | 分配器优化 | 同 `AttachContext` **不能并行 run**；reset 遗漏风险；与 kernel pool **层级混淆** | ⚠️ 不推荐 |
| **RS-B（RS′）** | **三层分开复用** + **`reload_batch_reset`** + **ReloadSessionPolicy** | 生产可选 per-file/per-batch/多轮；测试与生产同 API | 须定义 reset 不变量；store 归属需 API 收敛 | ⭐ **用户已授权采用** |
| **RS-C** | 维持 create/destroy + harness **`bind_store`** | 泄漏测例简单 | 无法表达「同 shell 多轮」生产路径 | 📋 **kOneShot 保留** |

#### 目标架构（RS′ · ASCII）

```text
┌─────────────────────────────────────────────────────────────────────────┐
│ 长生命周期层（进程 / attach 会话 · 与 KernelPool 对齐）                    │
│   AttachContext ── freeze ──► KernelPool warmup（已有 · 第21天）          │
│   BsAttachStore  ── 绑 ctx 或外部持有 ──► 跨 reload 复用 manifest epoch     │
├─────────────────────────────────────────────────────────────────────────┤
│ 短会话层（RES-IX 暂存 · 一次 reload 事务边界）                             │
│   ReloadBatchController（可长驻对象 · 须 reset/end）                       │
│     add_path* → run → [reset | destroy]                                  │
│   同一 ctx 上：**同一时刻仅一条 run 占写窗口**（第20天 B″）                  │
├─────────────────────────────────────────────────────────────────────────┤
│ 测试策略层（ReloadSessionPolicy · 非产品第二套池）                         │
│   kOneShot        · 1 URI → run → destroy（Day19 day 默认 · XIX-MEM-4）   │
│   kReuseController· reset → add → run × N（生产多轮 / smoke 可选）        │
│   kBatchNight     · 多 URI → PER_BATCH → 一次 run（Day19 night）          │
│   统计：iter = run 成功次数（≠ destroy 次数）                              │
└─────────────────────────────────────────────────────────────────────────┘
```

**与 RES-IX / 第21天 / 第19天关系**

| 关系 | 要点 |
|------|------|
| **RES-IX-5** | session 仍绑 **同一 `ReloadBatchController`**；**结束** = `reset` **或** `destroy`，须释放 path/IR/snapshot |
| **RES-IX-1～4** | store 长驻 **不** 改变「commit 前读者不可见」 |
| **第21天 A′′′** | KernelPool **已在 ctx 长驻**；RS′ **不** 引入第二套执行池 |
| **第19天 XIX-MEM** | smoke day 默认 **kReuseController**（**1500 run**）；arch_gap **kOneShot**；**XIX-MEM-10** 不变 |
| **第20天 B″** | 同 ctx **禁止** 多 controller **并行** `run`；RS-A 对象池 **无并行收益** |

#### 架构不变量（**RES-IX-17** · 已确认 · 2026-06-04）

**与 RES-IX-5 关系**：**RES-IX-17** 增补 session 边界中的 **store（L1）/ controller 暂存（L2）** 分层；**不替代** RES-IX-5 的 commit/abort 语义。

| 编号 | 架构不变量 | 状态 | 要点 |
|------|-------------|------|------|
| **RES-IX-17** | store 长驻 + controller session reset | ✅ 已确认 | **`BsAttachStore` 挂 `AttachContext`**；主 API **`reload_batch_reset`**；**`destroy(ctrl)` 不 close store**；禁止 controller 对象池 |

**落实检查清单（绑定 RES-IX-17 · 子任务 B 验收）**

| 编号 | 检查项 | 工程状态 | 要点 |
|------|--------|----------|------|
| **RS-CK-1** | 三层生命周期分离 | ✅ | **L1** ctx+pool+**persist_store**；**L2** controller；**L3** **ReloadSessionPolicy** |
| **RS-CK-2** | store 非 controller 独占 | ✅ | `close` 仅 **ctx teardown** 或显式 `ctx_close_persist_store` |
| **RS-CK-3** | reset 等价 abort 暂存 | ✅ | 释放 **全部** `PathWork`、IR、session_bytes；**禁止** 残留 uri_index |
| **RS-CK-4** | 写窗口互斥 | ✅ | 同 ctx **禁止** 并行 `run`（**XX-CONC**） |
| **RS-CK-5** | iter 统计口径 | ✅ | **iter = run 序号**；destroy **不计** iter（**XIX-MEM-4** 不变） |
| **RS-CK-6** | 禁止 controller 并行池 | ✅ | RS-A **不采用** |
| **RS-CK-7** | scheme 仍显式 | ✅ | **RES-IX-9**；policy **不** 隐式改 scheme |
| **RS-CK-8** | WAL purge coalesce | ✅ | **store_open** + **batch_commit** 增量 purge；PER_PATH 无 WAL append |
...
| 层 | 对象 | 生命周期 | 证据锚点 |
|----|------|----------|----------|
| L1 | `AttachContext` + `BsKernelPool` | freeze → warmup 一次 | `registry_bootstrap.cpp` L156-157 |
| L1 | `BsAttachStore` | **`AttachContext::persist_store`**；跨 reload | **`attach_context.*`**（**T-RS.7** ✅） |
| L2 | `ReloadBatchController` | 可多轮 `reset→add→run` | `reload_batch_controller.cpp` L42-64 |
| L3 | `ReloadSessionPolicy` | 测试 / 配置 | `Day19StressReloadLoopTest.cpp`（待抽象） |

##### 2️⃣ Session reset API（优于 controller pool）

**已确认公开 API（主路径 · RES-IX-17）**：

```c
/* 主 API：清空 L2 暂存；不 close ctx persist_store；不碰 attach_ctx / kernel_pool */
void bs_adapter_attach_reload_batch_reset(ReloadBatchController* ctrl);

/* ctx 级 store（挂点 A · 与 destroy(ctrl) 解耦） */
...
**刻意未启（本期）**：`begin_run` / `end_run` 薄包装 · `reload_batch_reopen`（易与 store 生命周期混淆）。

**`reset` 必须清零（RS-CK-3）**：`paths[]` · `uri_index` · `gated_ir` · `ir_snapshot_handle` · `session_bytes_used` · `outcome` · `write_window`（须已 `end`）· **不** `store_close`。

##### 3️⃣ ReloadSessionPolicy（测试 / 配置）

| Policy | 行为 | 用途 |
|--------|------|------|
| **kOneShot** | `create → add(1..N) → run → destroy` | **arch_gap** 泄漏/regression；**非** smoke day 默认 |
| **kReuseController** | `create 一次 → (reset → add → run)*N` | **smoke/ci/full day 默认**；生产多轮主路径 |
| **kBatchNight** | `create → add(N) → run → destroy`（或复用 ctrl） | Day19 **night 默认** · **PER_BATCH** |

配置入口：`day19_profile.h` 增 `reload_session_policy_day` / `_night`；可选 env `BS_DAY19_RELOAD_POLICY=one_shot|reuse|batch`（**smoke day 默认 `reuse`**）。

#### 用户确认的关键决策（RS′ · 2026-06-04 全部裁定点已确认）

| 选择项 | 用户选择 | 说明 |
|--------|---------|------|
| Controller 池化 vs session reset | **session reset（RS′）** | 不对齐 kernel pool checkout；语义 = RES-IX session 边界 |
| **Store 挂点** | **A · `AttachContext::persist_store`** | **`destroy(ctrl)` 不 close store**；`close` 归属 ctx |
| **API 命名** | **`reload_batch_reset` 为主 API** | 本期 **不** 引入 `begin/end` / `reload_batch_reopen` |
| **条文级别** | **直接升格 RES-IX-17** | 见上文 **RES-IX-17** + **RS-CK-1～8** 落实清单 |
| **Day19 smoke** | day **`kReuseController`**；night **`kBatchNight`** | **arch_gap** 另测 **kOneShot**；**XIX-MEM-10** 不变 |
| Controller 并行 | **不采用** | 同 ctx 写窗口 **串行**（第20天） |
| Controller pool（RS-A） | **不采用** | 见 **RS-CK-6** |

#### 落地建议（RS′ · 子任务 B 已执行 ✅）

**强制顺序**：**T-RS.0（API/不变量）→ T-RS.1（ctx store）→ T-RS.2（reset 实现）→ T-RS.3（Day19 policy）→ T-RS.4（arch_gap）**

| 落地项 | IMPL | 状态 | 影响范围 / 交付物 |
|--------|------|------|-------------------|
| **T-RS.0** | **IMPL-22-RS-00** | ✅ | `reload_batch_controller.h`：**`reload_batch_reset`** 声明 + **RES-IX-17** 文档 |
| **T-RS.1** | **IMPL-22-RS-01** | ✅ | `attach_context.*`：**`persist_store`（挂点 A）**；`destroy(ctrl)` **不** close store |
| **T-RS.2** | **IMPL-22-RS-02** | ✅ | `reload_batch_controller.cpp`：`reload_batch_reset` 实现 + 全量 path/IR 清理 |
| **T-RS.3** | **IMPL-22-RS-03** | ✅ | `Day19StressReloadLoopTest.cpp`：smoke day **`kReuseController`**；night **`kBatchNight`** |
| **T-RS.4** | **IMPL-22-RS-04** | ✅ | arch_gap：**AG-RS-RESET-1** / **AG-RS-STORE-1** / **AG-RS-ONE-SHOT-1** · `-L arch_gap` |
| **T-RS.5** | **IMPL-22-RS-05** | ✅ | `attach_store.cpp` WAL coalesce（**RS-CK-8** · open + batch_commit） |
| **T-RS.6** | **IMPL-22-RS-06** | ✅ | `docs/KERNEL_POOL_EXECUTOR_MODEL.md` § **RES-IX-17 / RS′** |
| **T-RS.7** | **IMPL-22-RS-07** | ✅ | 已移除 **`testing_bind_store`**；统一 **ctx persist_store** |

**实现要点（子任务 B 必读）**

1. **`reset` 与 `destroy` 分离**：`destroy` 仅当 controller 对象生命周期结束；**多轮 reload 用 `reset`**。  
2. **Store 挂点 A**：`ctx_open_persist_store` 单次 open；controller 从 ctx **borrow**；**禁止** `destroy(ctrl)` close store。  
3. **Day19**：smoke **day = kReuseController**；**night = kBatchNight**；**arch_gap** 覆盖 **kOneShot** + reset 泄漏；**XIX-MEM-10** 不变。  
4. **禁止** controller **并行池** 或 `-j 1` 全局锁掩盖 reset 遗漏。  
5. **`testing_bind_store`** 已移除（**T-RS.7** ✅）；统一 **`ctx_persist_store`**。

**验收标准（主任务审核用）**

| 项 | 标准 |
|----|------|
| **AG-RS-RESET-1** | 同一 ctrl **100 轮** reset→add→run **无** revision 泄漏 / IR 泄漏 |
| **AG-RS-STORE-1** | ctx store：**100 轮** **无** `store_open` 计数增加（或 purge coalesce 有界） |
| **Day19 ci** | `ctest -L day19` **仍 4/4+**；smoke day **kReuseController** 满足 **XIX-MEM-4/10** |
| **arch_gap** | **kOneShot** + **AG-RS-RESET-1** / **AG-RS-STORE-1** PASS |
| **regression** | `ReloadBatchControllerTest` / `AttachResilienceTest` **无退化** |
| **文档** | **RS-CK-1～8** 工程项标 ✅（主任务 2026-06-08） |

#### 方案解决的核心问题（RS′）

| 问题 | 解决方式 |
|------|---------|
| 频繁 `store_open` + WAL purge | **ctx persist_store** + **purge coalesce**（**RS-CK-8**） |
| Controller 与 pool 叙事冲突 | **分层**：pool=执行；controller=编排 session；**禁止** controller pool |
| Day19 与生产路径分裂 | **ReloadSessionPolicy** 同一套 API |
| 测试写死 create/destroy | smoke **kReuseController**；arch_gap **kOneShot** |
| destroy 关闭 store | **挂点 A** + **RES-IX-17** / **RS-CK-2** |

#### 架构短板登记（RS′ 专题）

| ID | 短板主题 | 架构影响 | 建议补齐时机 | 状态 | 参见 |
|----|-----------|----------|--------------|------|------|
| **RS-G-01** | controller 绑 store 生命周期 | 重复 open/purge | **T-RS.1～2** | ✅ | **ctx persist_store** · **AG-RS-STORE-1** |
| **RS-G-02** | 无公开 `reset` API | 生产只能 destroy | **T-RS.0～2** | ✅ | **IMPL-22-RS-00** · **AG-RS-RESET-1** |
| **RS-D-01** | Day19 policy 分裂 | smoke vs arch_gap | **已裁定** | ✅ | day **kReuse**；gap **kOneShot** |
| **RS-D-02** | RS-A controller pool 误用风险 | 错误并行假设 | 文档 **RS-CK-6** | ✅ | 本章 |

#### 研析附录（RS′ · 结构化对比）

| 对比维度 | RS-A Controller 池 | **RS′ reset + 三层** | RS-C create/destroy |
|----------|-------------------|----------------------|---------------------|
| 同 ctx 并行 run | ❌ 仍串行 | ❌ 仍串行 | ❌ 仍串行 |
| store 复用 | 🟡 需额外 bind | ✅ **一等公民** | ❌ 除非 bind_store |
| RES-IX-5 对齐 | 🟡 checkout 语义模糊 | ✅ session reset | ✅ 清晰 |
| 泄漏测例 | 🟡 reset 遗漏难查 | ✅ AG-RS-RESET-1 | ✅ destroy 路径 |
| Day19 iter 口径 | 🟡 | ✅ policy 分离 | ✅ kOneShot |
| 证据 | `kernel_pool.c` submit | `reload_batch_controller.cpp` L143-164 | `Day19StressReloadLoopTest.cpp` L185-227 |
| Spring Cloud 类比 | `@RefreshScope` bean 池 ❌ | **`ApplicationContext` 长驻 + `@RefreshScope` 单次** 📋 弱类比 | 每次 new context ❌ |

**研读来源（BlessStar 现状 · 非标准源对照）**

| 来源 | 路径 |
|------|------|
| Session 写窗口 | `attach_session.cpp` · `DAY20_CONCURRENCY_MODEL.md` |
| Store open/close | `attach_store.cpp` L351-377 · L143-164 `destroy` |
| Pool warmup | `registry_bootstrap.cpp` L156-157 |
| Day19 harness | `Day19StressReloadLoopTest.cpp` · **XIX-MEM-4/5/8** |
| 架构迭代规则 | `.cursor/rules/architecture-shortcoming-iteration.mdc` |

### 轮次元信息（第22天 · A′ + R-A + RS′ 索引）

| 字段 | 值 |
|------|-----|
| 轮次 | 第 22 天 · **崩溃恢复 A′ + 全量回归 R-A + Reload Session RS′** |
| 用户已裁决 | **A′**；**R-A**；**T19.12-C**；**RS′** 全裁定点（2026-06-04）：挂点 **A** · API **`reset`** · **RES-IX-17** · smoke day **kReuse** / night **kBatch** |
| MVP 工程范围 | **T-REC.0～5** + **T-REG.0～4** + **T-RS.0～4**（P0）；**T-RS.5～7**（文档/收尾） |
| 刻意未启 | **replay exec**；**auto_reload**；**RS-A controller 对象池**；`begin_run`/`end_run` |
| 未决 | —（RS′ 裁定点已闭合） |
| 下一步 | 子任务 B **T-RS.0→4**（与 T-REC/T-REG 并行时须 **attach_integration** 锁序） |
...
| RES-G-04 | 读者未绑持久层 | **第12天→22天** | attach | 🟡 | `get_snapshot_*` 已有；manifest 绑 revision 待 **T-REC-G03-4** | 第22天 § **REC-G-03 闭合** |
...
| **R21-05** | P2 attach TLS / 写窗口与 pool 正交 | **第21天** | attach | ✅ | **arch_gap** 测例 | **21.12～21.14** | 第21天 § **R21-05** |
| **REC-G-01** | 无进程级 recover API | **第22天** | attach | ✅ | `attach_recover.h` | **T-REC.0～5** | 第22天 § **REC-G-01** |
| **REC-G-02** | exec 后 persist 前 crash 语义 | **第21天→22天** | orch | ✅ | `reload_batch_controller` Phase B/C · WAL PHASE | **T-REC-6～7** | 第22天 § **REC-G-02** |
| **REC-G-03** | 持久 vs 运行时双真相 | **第12天→22天** | attach | 🟡 | **RES-D-05** | **T-REC-G03-0～4**；ADR ✅ | 第22天 § **REC-G-03 闭合** |
| **REG-G-01** | 无一键回归文档入口 | **第22天** | test/docs | ✅ | `contract_gate_runner` | **DAY22_REGRESSION_CHAIN** | 第22天 § **REG-G-01** |
| **REG-G-02** | 覆盖率无汇总 | **第22天** | test | ✅ | `collect_coverage.py` | **T-REG.2** | 第22天 § **REG-G-02** |
| **REG-G-03** | **T19.12** 契约 | **第19天→22天** | contract | ✅ | **C-TST-MEM-1** rule-only | **T-REG-1** | 第22天 § T19.12-C |
...
| IMPL-22-RS-00 | **T-RS.0** · **`reload_batch_reset`** 主 API | 第22天 | orch | ✅ | `reload_batch_controller.h` 公开 API | **RES-IX-17** + **RS-CK-3** | 项目修改记录 **22.11** |
| IMPL-22-RS-01 | **T-RS.1** · **挂点 A** `ctx persist_store` | 第22天 | attach | ✅ | `attach_context.*` | `destroy(ctrl)` 不 close store | 项目修改记录 **22.11** |
| IMPL-22-RS-02 | **T-RS.2** · reset 实现 + gc 全覆盖 | 第22天 | orch | ✅ | `reload_batch_reset` 全量清理 | **AG-RS-RESET-1** | 项目修改记录 **22.11** |
| IMPL-22-RS-03 | **T-RS.3** · Day19 policy：smoke **kReuse** / night **kBatch** | 第22天 | test | ✅ | `Day19StressReloadLoopTest.cpp` | **XIX-MEM-10** 不变 | 项目修改记录 **22.11** |
| IMPL-22-RS-04 | **T-RS.4** · arch_gap：**kOneShot** + AG-RS | 第22天 | test | ✅ | `-L arch_gap` **5/5** | **AG-RS-*** | 项目修改记录 **22.11** |
| IMPL-22-RS-05 | **T-RS.5** · WAL purge coalesce | 第22天 | persist | ✅ | open + batch_commit | 收口 **RS-CK-8** | 项目修改记录 **22.12** |
| IMPL-22-RS-06 | **T-RS.6** · KERNEL_POOL doc 与 RS′ | 第22天 | docs | ✅ | § RES-IX-17 | Day19 policy 表 | 项目修改记录 **22.12** |
| IMPL-22-RS-07 | **T-RS.7** · 移除 testing_bind_store | 第22天 | orch | ✅ | hook 已删 | ctx persist_store | 项目修改记录 **22.12** |
| IMPL-REC-G03-00 | **T-REC-G03-0** · **C-ATTACH-SYNC-1～6** 契约 | 第22天 | contract | 🟪 | `docs/contracts/attach/` | validate+compile | § **REC-G-03 闭合** |
| IMPL-REC-G03-00b | **T-REC-G03-0** · **GATE-ATTACH-SYNC-PROD** | 第22天 | gates | 🟪 | `check_attach_sync_prod_paths.py` | 生产禁裸 persist | § **REC-G-03 闭合** |
| IMPL-REC-G03-01 | **T-REC-G03-1** · `post_config_sync` + pipeline reset | 第22天 | adapter | 🟪 | `attach_config.*` | **REC-G03-3a** | § **REC-G-03 闭合** |
| IMPL-REC-G03-02 | **T-REC-G03-2** · IrSnapshot remove + **WIRE-07** | 第22天 | adapter+test | 🟪 | `reload_batch_controller` · **AG-WIRE-FULL-1** | **REC-G03-3b** | § **REC-G-03 闭合** |
| IMPL-REC-G03-03 | **T-REC-G03-3** · **AG-NOTIFY-FLUSH-1** | 第22天 | test+docs | 🟪 | `AttachNotifyFlushTest` | **REC-G03-4** | § **REC-G-03 闭合** |
| IMPL-REC-G03-04 | **T-REC-G03-4** · **RES-G-04** 读者 revision | 第22天 | adapter | 🟪 | `attach_session.*` | **AG-READER-REV-*** | § **REC-G-03 闭合** |
