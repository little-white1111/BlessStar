...
> **状态**：⭐ **方案 A′ + R-A** 及 **REC-A'-1～14 / REG-A'-1～5** 已用户确认（2026-06-04）；**T19.12-C** 已裁定；子任务 B **T-REC / T-REG** 🟪 待执行；**待子任务 B 工程落地与主任务审核**。  
...
| **RES-D-05** 磁盘 vs 运行时 | Step-② cold reload 对齐 CM |
...
| 测试 | `bs_test_attach_recover_{cold,fsm,sidecar}` · `-L recover` ✅ **3/3** |
...
| **REC-G-03** | **RES-D-05** 双真相 | 错账窗口 | **T-REC.2** | 🟡 | Step-② |
| **REC-O-01** | RECOVERING 并发读 | 脏读 | **AG-REC-CONC-1** | 🟪 | **REC-A'-5** |
...
| 字段 | 值 |
|------|-----|
| 轮次 | 第 22 天 · **崩溃恢复 A′ + 全量回归 R-A** |
| 用户已裁决 | **A′**；**REC-A'-7 rollback**；**recover 显式两步**；**R-A**；**T19.12-C**（2026-06-04） |
| MVP 工程范围 | **T-REC.0～5** + **T-REG.0～4** |
| 刻意未启 | **replay exec**；**auto_reload** |
| 下一步 | 主任务审核 → 子任务 B 按 **§ 落地建议** 顺序执行 |
...
| **REC-G-03** | 持久 vs 运行时双真相 | **第12天→22天** | attach | 🟡 | **RES-D-05** | **T-REC-2**；C 可选 | 第22天 § **REC-G-03** |
