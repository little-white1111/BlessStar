...
// OrderConfigAdapter 订单管理 域配置的 BlessStar 适配器
// 内置三阶段降级: ConfigReader实时查询 → LastKnownGood缓存 → 硬编码默认值
type OrderConfigAdapter struct {
	reader          ports.ConfigReader
	lastKnownCache  sync.Map
	hardcodedDefaults  map[string]interface{}
}

// NewOrderConfigAdapter 创建 OrderConfigAdapter 适配器实例
// reader 参数是配置读取器，由业务方注入（可为 CachedReader、HTTPReader 等实现）
func NewOrderConfigAdapter(reader ports.ConfigReader) ports.OrderConfig {
	return &OrderConfigAdapter{
		reader: reader,
		hardcodedDefaults: map[string]interface{}{
			"StatusValues": "{\"0\":\"pending_payment\",\"1\":\"paid\",\"2\":\"shipped\",\"3\":\"delivered\",\"4\":\"completed\",\"-1\":\"cancelled\"}",
		},
	}
}

func (a *OrderConfigAdapter) StatusValues(ctx context.Context) (string, error) {
	// 第1阶段: ConfigReader 实时查询（SHM / HTTP / 环境变量等）
	val, err := a.reader.Get(ctx, "/config/douyin-mall/order/status/values")
	if err == nil {
