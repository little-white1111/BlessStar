...
interface RawSchema {
  ...
  version?: string;
  fields?: RawField[];
  'x-extensions'?: Record<string, unknown>;
}

/** 从 config-schema.yaml 解析 Schema */
export function parseSchema(yamlContent: string): ConfigSchema {
  const raw = yaml.load(yamlContent) as RawSchema;
...
function buildHeader(sourceFile: string): string {
  return [
    '/**',
...
export function generateTypes(schema: ConfigSchema, sourceFile = 'config-schema.yaml'): string {
  const lines: string[] = [];
...
export function generatePort(schema: ConfigSchema, sourceFile = 'config-schema.yaml'): string {
  const lines: string[] = [];
  lines.push(buildHeader(sourceFile));
...
/** 生成 Mock Adapter */
export function generateMockAdapter(schema: ConfigSchema, sourceFile = 'config-schema.yaml'): string {
  const lines: string[] = [];
  lines.push(buildHeader(sourceFile));
  lines.push(`import type { LivestyleConfig } from './livestyle-config.types';`);
...
/** 生成 LocalFileAdapter */
export function generateLocalFileAdapter(schema: ConfigSchema, sourceFile = 'config-schema.yaml'): string {
  const lines: string[] = [];
  lines.push(buildHeader(sourceFile));
...
function formatDefaultValue(value: unknown, tsType: string): string {
  if (value === undefined || value === null) return '';
  if (tsType === 'string') return `'${String(value)}'`;
  return String(value);
}

function formatDefaultValueLiteral(value: unknown, rawType: string): string {
  if (value === undefined || value === null) {
    if (rawType === 'BOOL') return 'false';
    if (rawType === 'I32') return '0';
    return "''";
  }
...
function groupByModule(fields: ConfigFieldDefinition[]): Record<string, ConfigFieldDefinition[]> {
  const groups: Record<string, ConfigFieldDefinition[]> = {};
