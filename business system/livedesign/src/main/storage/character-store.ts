...
export class CharacterStore {
  ...
  private cache: Map<string, CharacterCard> = new Map();
   ...
   * 获取默认角色卡目录
   */
  private getDefaultCharactersDir(): string {
   ...
   * 确保角色卡目录存在
   */
  private ensureCharactersDir(): void {
   ...
   * 从文件加载角色卡
   */
  private loadCardFromFile(filePath: string): CharacterCard {
    const raw = fs.readFileSync(filePath, 'utf-8');
    const parsed = JSON.parse(raw) as CharacterCard;
    return parsed;
  }
  ...
  getCharacter(characterId: string): CharacterCard | null {
    ...
    try {
      ...
      if (!validation.valid) {
        console.warn(
          `[CharacterStore] 角色卡校验失败: ${characterId}`,
          validation.errors,
        );
        // 即使校验失败也返回卡片，由调用方处理
      }
  ...
  validateCharacter(characterId: string): CharacterCardValidation {
    ...
    return validateCharacterCard(card);
  }
  ...
  switchCharacter(characterId: string): CharacterCard | null {
    ...
    if (!validation.valid) {
      console.error(
        `[CharacterStore] 切换角色失败，角色卡校验不通过: ${characterId}`,
        validation.errors,
      );
      return null;
    }
    ...
    // 持久化当前角色（架构不变量 #5）
    configStore.set('character.current', characterId);
  ...
  getCurrentCharacter(): CharacterCard | null {
    const currentId = configStore.get<string>('character.current');
  ...
  getCurrentCharacterId(): string | null {
    return configStore.get<string>('character.current') ?? null;
  }
  ...
  createDefaultCharacterIfEmpty(): void {
    ...
    if (files.length > 0) return; // 已有角色卡
