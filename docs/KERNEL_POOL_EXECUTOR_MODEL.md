...
## 停止语义（P1-A / P2-A · 用户闭合裁定 · IMPL-22-HANG 演进）

| 场景 | API / 行为 |
|------|------------|
| 单 Kernel slot | `bs_kernel_stop` → **STOPPING** → 拒新 job → **P1-A** 等在跑 job 完成 → flush/join → 唤醒 waiter（`BS_KERNEL_ERR_STOPPED`） |
| 整个 Pool **`destroy`** | `bs_kernel_pool_destroy` → **POOL_DRAINING** → reject submit → **cancel** busy job → idle slot 销毁；**busy slot 不 wait**（随 ctx 泄漏隔离） |
| Pool **`reset_all_pipelines`** | 孤儿化 busy slot → **`warmup` 重建 pool**（非阻塞等待 busy 归零） |
| **execute 超时** | **P2-A**：MVP **不提供**；第21天 **不**实现超时 API |
| **协作式 cancel** | exec **入口**协作检查 `exec_cancel_requested` / `bs_kernel_request_exec_cancel`（IMPL-22-HANG）；**stage 级** cancel 仍为二期 |

> **与第21天原文差异**：第21天闭合裁定写「全 slot P1-A drain」；第22天 **IMPL-22-HANG** 将 **ctx destroy** 路径改为 **destroy-no-wait**，避免 pool busy 与 attach teardown 形成无超时等待链。单 slot `bs_kernel_stop` 仍 **P1-A** drain。契约 **C-KERNEL-POOL-1** 已增补 `destroy_no_wait` / `reset_rebuild_orphans_busy`。
...
**交叉引用**：`架构方案选择记录.md` § 第22天 **RS′** 与 **hang 长时间挂起修复措施**；`docs/DAY20_CONCURRENCY_MODEL.md`（写窗口互斥与 **XX-CONC-5** 销毁序）。
