...
{
    IRInstruction* instr = bs_ir_instruction_create(nullptr, nullptr);
...
{
    IRInstruction* instr = bs_ir_instruction_create("", "");
    assert(instr != nullptr);
    bs_ir_instruction_destroy(instr);
...
static void test_IR_LongStrings()
{
    ...
    IRInstruction* instr = bs_ir_instruction_create(long_str.data(), long_str.data());
    assert(instr != nullptr);
    bs_ir_instruction_destroy(instr);
...
static void test_IRMetadata_NullKey()
{
    ...
    IRMetadata*    meta  = bs_ir_metadata_create(nullptr, "value");
    assert(meta != nullptr);
...
{
    IRInstruction* instr = bs_ir_instruction_create("test", "name");
    IRMetadata*    meta1 = bs_ir_metadata_create("key", "value1");
    IRMetadata*    meta2 = bs_ir_metadata_create("key", "value2");
...
static void test_IRList_Empty()
{
    ...
    assert(bs_ir_instruction_list_size(list) == 0);
    assert(bs_ir_instruction_list_get(list, 0) == nullptr);
...
static void test_IRList_LargeSize()
{
    ...
    assert(bs_ir_instruction_list_size(list) == N);
    bs_ir_instruction_list_destroy(list);
...
static void test_IRList_IndexOutOfBounds()
{
    ...
    assert(bs_ir_instruction_list_get(list, -1) == nullptr);
    assert(bs_ir_instruction_list_get(list, 1) == nullptr);
    assert(bs_ir_instruction_list_get(list, 100) == nullptr);
