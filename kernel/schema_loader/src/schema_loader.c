...
struct bs_schema_loader* bs_schema_loader_create(const char* yaml_path)
{
    ...
    if (yaml_path) {
        ...
        if (!loader->yaml_path) { free(loader); return NULL; }
    }
...
static int check_required_fields(const struct bs_schema* schema)
{
    ...
    for (size_t i = 0; i < BS_GOLD_STANDARD_V1_COUNT; i++) {
        if (!BS_GOLD_STANDARD_V1[i].required) continue;

        int found = 0;
        for (size_t j = 0; j < schema->field_count; j++) {
            if (strcmp(schema->fields[j].qualified_name,
                       BS_GOLD_STANDARD_V1[i].qualified_name) == 0)
            {
                found = 1;
                break;
            }
    ...
    for (size_t i = 0; i < BS_GOLD_STANDARD_V1_COUNT; i++) {
        ...
        if (!found) {
            ...
            if (BS_GOLD_STANDARD_V1[i].default_value) {
                struct bs_schema_field* new_fields = (struct bs_schema_field*)
                    realloc(schema->fields,
                            (schema->field_count + 1) * sizeof(struct bs_schema_field));
...
int bs_schema_loader_reload(struct bs_schema_loader* loader)
{
    ...
    bs_log_emit(BS_LOG_DOMAIN_SCHEMA_LOADER, BS_LOG_INFO,
                 "SchemaLoader: reload triggered for '%s'",
    ...
    bs_log_emit(BS_LOG_DOMAIN_SCHEMA_LOADER, BS_LOG_INFO,
                 "SchemaLoader: reload successful (%zu fields)",
                 temp->field_count);
...
int bs_schema_loader_reload_bundled(struct bs_schema_loader* loader)
{
    ...
    bs_log_emit(BS_LOG_DOMAIN_SCHEMA_LOADER, BS_LOG_INFO,
                 "SchemaLoader(bundled): reload successful (%zu fields)",
                 temp->field_count);
