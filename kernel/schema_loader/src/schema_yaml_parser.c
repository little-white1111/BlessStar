...
static int parse_kv_line(const char* line, char** key, char** value)
{
    ...
    while (key_len > 0 && (unsigned char)t[key_len - 1] <= ' ') key_len--;
...
int bs_schema_yaml_parse(const char* yaml_path, struct bs_schema** out)
{
    ...
    *out = NULL;
    ...
    if (!schema) { fclose(fp); return -1; }

    schema->fields = (struct bs_schema_field*)calloc(max_fields + 1, sizeof(struct bs_schema_field));
    if (!schema->fields) { free(schema); fclose(fp); return -1; }
    ...
    if (!schema->version) { bs_schema_free(schema); return -1; }
...
{
    if (!s) return BS_SCHEMA_TYPE_STR;
    if (strcmp(s, "STR")  == 0) return BS_SCHEMA_TYPE_STR;
    if (strcmp(s, "I32")  == 0) return BS_SCHEMA_TYPE_I32;
    if (strcmp(s, "I64")  == 0) return BS_SCHEMA_TYPE_I64;
    if (strcmp(s, "F64")  == 0) return BS_SCHEMA_TYPE_F64;
    if (strcmp(s, "BOOL") == 0) return BS_SCHEMA_TYPE_BOOL;
    if (strcmp(s, "ARR")  == 0) return BS_SCHEMA_TYPE_ARR;
    if (strcmp(s, "OBJ")  == 0) return BS_SCHEMA_TYPE_OBJ;
    if (strcmp(s, "ENUM") == 0) return BS_SCHEMA_TYPE_ENUM;
...
static void strip_outer_quotes(const char* s, char* dst, size_t dst_size)
{
    ...
    {
        size_t inner = len - 2;
        if (inner >= dst_size) inner = dst_size - 1;
...
int bs_schema_yaml_parse_bundled(const char* yaml_path, struct bs_schema** out)
{
    ...
    *out = NULL;
    ...
    if (max_fields > 0) {
        schema->fields = (struct bs_schema_field*)calloc(max_fields, sizeof(struct bs_schema_field));
        if (!schema->fields) { free(schema); fclose(fp); return -1; }
    }
    ...
    enum { ST_HEADER, ST_IN_FIELDS, ST_FIELD_ITEM } state = ST_HEADER;
    size_t idx = 0;
    int in_contract = 0;
    ...
    while (fgets(buf, sizeof(buf), fp)) {
        ...
        case ST_HEADER:
            if (strncmp(t, "version:", 8) == 0) {
                ...
                if (!schema->version) { bs_schema_free(schema); fclose(fp); return -1; }
            }
            if (strcmp(t, "fields:") == 0) {
                state = ST_IN_FIELDS;
            }
