...
// GenerateGateConfigs 生成 gate_configs.h 门禁配置文件
func (g *GoBackend) GenerateGateConfigs(biz *types.BizSystem, gateConfigs []types.GateConfig) (*backend.File, error) {
	...
	b.WriteString("//\n")
	b.WriteString("// gate_configs.h — 门禁配置规则\n")
	b.WriteString("// 由 config-schema.yaml contract 段自动生成\n")
	b.WriteString("#ifndef GATE_CONFIGS_H\n")
	b.WriteString("#define GATE_CONFIGS_H\n\n")
	b.WriteString("#include <stdint.h>\n")
	b.WriteString("#include <stdbool.h>\n\n")
	b.WriteString("#ifdef __cplusplus\nextern \"C\" {\n#endif\n\n")

	// Group by field key and gate type
	...
	for _, fk := range fieldKeys {
		entries := fieldMap[fk]
		b.WriteString(fmt.Sprintf("// Gate configs for: %s\n", fk))
		for _, e := range entries {
			switch e.GateType {
			case "RANGE":
				b.WriteString(fmt.Sprintf("#define GATE_%s_%s %s\n", sanitizeGateMacro(fk), strings.ToUpper(e.ParamKey), e.ParamVal))
			case "DEPENDENCY":
				macroName := fmt.Sprintf("GATE_%s_DEP", sanitizeGateMacro(fk))
				b.WriteString(fmt.Sprintf("#define %s %q\n", macroName, e.ParamVal))
			case "APPROVAL":
				b.WriteString(fmt.Sprintf("#define GATE_%s_APPROVAL_REQUIRED 1\n", sanitizeGateMacro(fk)))
			case "SLO_WARNING":
				b.WriteString(fmt.Sprintf("#define GATE_%s_SLO %q\n", sanitizeGateMacro(fk), e.ParamVal))
			}
		}
		b.WriteString("\n")
	}
	...
	b.WriteString("#ifdef __cplusplus\n}\n#endif\n\n")
	b.WriteString("#endif /* GATE_CONFIGS_H */\n")
...
// sanitizeGateMacro converts a config key to a gate macro name
func sanitizeGateMacro(key string) string {
...
func (g *GoBackend) GenerateTestCases(biz *types.BizSystem, testCases []string) (*backend.File, error) {
	if len(testCases) == 0 {
		return nil, nil
	}
	...
	b.WriteString(fmt.Sprintf("// Business System: %s (%s)\n", biz.DisplayName, biz.BizID))
	b.WriteString("//\n")
	b.WriteString("// config_boundary_test.go — 配置边界测试用例\n")
	b.WriteString("// 由 config-schema.yaml contract 段（range + dependencies）自动生成\n\n")
	b.WriteString(fmt.Sprintf("package %s\n\n", PackageNameFromBiz(biz.BizID)))
	b.WriteString("import (\n\t\"testing\"\n)\n\n")
	...
	return &backend.File{
		Path:    "test_cases/config_boundary_test.go",
		Content: b.String(),
	}, nil
}

// GenerateObservability 生成可观测性规则文件
func (g *GoBackend) GenerateObservability(biz *types.BizSystem, rules []string) (*backend.File, error) {
	if len(rules) == 0 {
		return nil, nil
	}
	...
	b.WriteString(fmt.Sprintf("# Business System: %s (%s)\n", biz.DisplayName, biz.BizID))
	b.WriteString("#\n")
	b.WriteString("# config_alerts.yml — Prometheus 告警规则\n")
	b.WriteString("# 由 config-schema.yaml contract.slo_impact 自动生成\n\n")
	b.WriteString("groups:\n")
	b.WriteString(fmt.Sprintf("  - name: %s_config_slo\n", biz.BizID))
	...
	return &backend.File{
		Path:    "observability/config_alerts.yml",
		Content: b.String(),
	}, nil
}
