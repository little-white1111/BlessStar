...
int main()
{
    ...
    BsStatus            st =
        bs_config_parse_bytes(reinterpret_cast<const uint8_t*>(kGolden), strlen(kGolden), &result);
  ...
  "instructions": [{"type":"not_in_manifest","name":"x"}]
    ...
    st = bs_config_parse_bytes(reinterpret_cast<const uint8_t*>(kBadType), strlen(kBadType),
                               &result);
    assert(bs_status_is_ok(st));
    assert(bs_adapter_requirement_filter_verify_instructions(result.instructions,
                                                             result.active_requirements) != 0);
