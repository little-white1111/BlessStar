...
int main()
{
    ...
    ReloadBatchController* ctrl = bs_adapter_attach_reload_batch_create(2);
    assert(ctrl != nullptr);
    ...
    assert(bs_adapter_attach_reload_batch_add_path(ctrl, "file:///a") == 0);
    assert(bs_adapter_attach_reload_batch_add_path(ctrl, "file:///b") == 0);
    assert(bs_adapter_attach_reload_batch_add_path(ctrl, "file:///c") == -1);
    ...
    bs_adapter_attach_reload_batch_set_read_fn(ctrl, mock_read, &read_ctx);
    bs_adapter_attach_reload_batch_set_gate_fn(ctrl, mock_gate, &gate_ctx);
    day12_wire_reload_defaults(ctrl);

    assert(bs_adapter_attach_reload_batch_add_path(ctrl, "file:///gated") == 0);
    ...
    bs_adapter_attach_reload_batch_set_read_fn(ctrl, mock_read, &read_ctx);
    bs_adapter_attach_reload_batch_set_gate_fn(ctrl, mock_gate_pass, nullptr);
    bs_adapter_attach_reload_batch_set_max_retry(ctrl, 1);
    day12_wire_reload_defaults(ctrl);

    assert(bs_adapter_attach_reload_batch_add_path(ctrl, "file:///retry") == 0);
    assert(bs_adapter_attach_reload_batch_run(ctrl) == 0);
    assert(bs_adapter_attach_reload_batch_outcome(ctrl) == BATCH_ALL_OK);
    assert(read_ctx.calls == 2);
